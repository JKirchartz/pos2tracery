...
exports.handler = function merge(args) {
  ...
  if (input_A, input_B) {
    let output_json = JSON.parse(utils.fileOrString(input_A));
    let file_B = JSON.parse(utils.fileOrString(input_B));
      ...
      if (output) {
        ...
        fs.writeFile(output,
          ...
          function (err) {
            ...
            log(0)('wrote grammar to %s', output);
            process.exit(0);
          });
      } else {
        log(0)(JSON.stringify(output_json, null, 2));
        process.exit(0);
      }
    } else {
