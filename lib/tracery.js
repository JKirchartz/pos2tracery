...
exports.builder = function (yargs) {
  return yargs.positional("input", {
    ...
    "custom_modifiers": {
      alias: "cm",
      description: "javascript file containing custom modifiers e.g. `export.modules = { <list of modifier functions> }",
      type: "string",
      nargs: 1
    },
    "o": {
    ...
    "evaluate": {
      alias: "e",
      describe: "evaluate tracery as javascript template (write javascript inside ${} in tracery)",
      default: false,
      type: "boolean"
    },
    "verbose": {
  ...
  }).help().alias('h', 'help').coerce(["custom_modifiers"], path.resolve);
};
...
exports.handler = function generate(args) {
  ...
  if (args.custom_modifiers) {
    log(1)("adding custom modifiers");
    grammar.addModifiers(require(args.custom_modifiers).modules);
  }

  for (var r=args.repeat, tmp; r>0; r--) {
  ...
  if (args.evaluate) {
    returning = new Function('return `' + returning + '`;')();
  }
