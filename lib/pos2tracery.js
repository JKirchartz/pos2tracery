...
exports.handler = function pos2tracery(args) {
  ...
  log(3)("commandline options:", args);
  ...
  const outputFile = (output) => {
    if (output && args.output) {
      fs.writeFile(args.output,
        ...
        function (err) {
          if (err) {return log(1)('everything sucks because: ', err);}
          log(1)('wrote grammar to %s', args.output);
          process.exit(0);
        });
    } else {
      log(0)(output);
  ...
  const parseSentence = (str) => {
    ...
    log(3)("tagged sentence:", taggedSentence);
  ...
  const parseCorpus = (file) => {
    ...
    sentences = sentences.map(parseSentence).filter((s) => s.length > 1);
  ...
  if (args.input) {
    var file = utils.fileOrString(args.input);
    if (file.length) {
      parseCorpus(file);
    } else {
      log(0)(file.error);
